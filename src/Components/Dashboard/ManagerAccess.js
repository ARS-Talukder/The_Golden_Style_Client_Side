...
const ManagerAccess = () => {
    ...
    const { data: users, isLoading: usersLoading, refetch } = useQuery('users', () => fetch('https://the-golden-style-server.onrender.com/users', {
        method: 'GET',
        headers: {
        ...
        <div>
            ...
            <div className="overflow-x-auto px-2 my-4">
                <table className="table w-full">
                    ...
                    <tbody>
                        {
                            users?.map((user, index) => <ManagerAccessRow key={index} index={index} user={user} refetch={refetch}></ManagerAccessRow>)
                        }
